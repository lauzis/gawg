...
- **Extra entries via invite links** — after submitting the entry form each participant receives a personal invite link (`?gwag-giveaway={uuid}&invite={participant-uuid}`). Any unique IP address that visits the site via that link increments the inviting participant's entry count for that giveaway by 1; repeat visits from the same IP are ignored. Entry counts are shown in the Participants admin list and in the "Entries & Invite Links" panel on each participant's edit screen.
