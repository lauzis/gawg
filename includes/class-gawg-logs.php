...
class GAWG_Logs {
	...
	public static function panel() {
		...
		if ( ! $logger || ! class_exists( '\Lauzis\WpPackages\Logs\Viewer' ) ) {
			...
			return '<p class="description">'
				. esc_html__( 'The log reader needs a newer copy of the shared package than the one running.', 'gawg' )
				. '</p>';
		}

		$viewer = new \Lauzis\WpPackages\Logs\Viewer( $logger, array( 'clear' => 'gawg_clear_logs' ) );
	...
	public static function handle_clear() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You are not allowed to do that.', 'gawg' ) );
		}

		check_admin_referer( 'gawg_clear_logs' );

		self::add( 'logs', 'Log cleared from the settings page.', array( 'user' => get_current_user_id() ) );
		self::clear();
